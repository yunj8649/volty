# Instagram Toon Prompt Guide

AI 이미지 작업은 기존 Volty 디자인을 확장하는 용도로만 사용한다. 캐릭터를 새로 해석하지 않는다.
...
```text
Create a 4-panel Instagram toon layout for Volty. The first panel shows empathy with an electrician exam student situation. The second panel shows one small study problem. The third panel shows one simple solution. The final panel gives warm encouragement. Use flat design, rounded shapes, pastel colors, minimal background, no gradient, no realistic effects. Leave clean empty space for Korean text to be added later.
```
